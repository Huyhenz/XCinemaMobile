...
class TrailerDialog extends StatefulWidget {
  ...
  const TrailerDialog({super.key, required this.trailerUrl});

  @override
  State<TrailerDialog> createState() => _TrailerDialogState();

  static Future<void> show(BuildContext context, String trailerUrl) {
    return showDialog(
      context: context,
      barrierColor: Colors.black87,
      barrierDismissible: true,
      builder: (context) => TrailerDialog(trailerUrl: trailerUrl),
    );
  }
}
...
class _TrailerDialogState extends State<TrailerDialog> {
  ...
  Future<void> _checkInternetAndInitialize() async {
    ...
    if (!_hasInternet) {
      setState(() {
        _errorMessage = 'Không có kết nối internet. Vui lòng kiểm tra kết nối mạng.';
        _isLoading = false;
      });
      return;
    }
